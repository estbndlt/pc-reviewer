...
Cleanup plans (LLM output): YAML ranked by GB freed, with rationale, risk level, dry-run commands, verification, rollback.
...
Type: Remote MCP (WebSocket)

URL: wss://<your-tunnel-host>/mcp
...
ChatGPT: Explore → Create a GPT → Configure

Connectors: enable PC Reviewer.
...
Denylist & allowlist. We refuse core system paths and user content by default; only allow known cache/artifact dirs.
